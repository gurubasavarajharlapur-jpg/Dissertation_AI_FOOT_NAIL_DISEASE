...
c.push(P('Attention localises to the toes and nail plates with the image margins cold, consistently across every example inspected, and the overlays were generated for misclassified predictions as well as correct ones rather than curating successes.'));
c.push(IMG('gradcam.jpg', 540, 655));
c.push(fcap('5.3', 'Grad-CAM overlays for MobileNetV2 on test predictions, showing correct predictions in green and misclassified ones in red; the warmer the colour, the more that region drove the prediction.'));
c.push(P('Two things in the figure are worth drawing out. In the wound panel a measuring ruler lies alongside the lesion, and it attracts no attention at all: a ruler is exactly the sort of clinical artefact a model could learn to key on, and this is the shortcut the ablation in Section 5.6.3 tests for numerically. And most of the misclassifications carry low confidence, between 0.56 and 0.74, which places them below the abstention threshold of 0.787 and means the mechanism in Section 5.5 would decline them rather than answer. The two that would survive it are the confident errors, a wound read as a nail infection at 0.99 and a nail infection read as healthy at 0.86, which is the limitation Section 6.4.3 sets out.'));
...
c.push(fcap('5.4', 'Controlled occlusion at 20-pixel border width. The interior control removes the same number of pixels as the border arm and differs only in location.'));
...
c.push(fcap('5.5', 'Each subject photographed against both backgrounds. Lines join the two photographs of the same feet.'));
