...
c.push(P('Two architectures were compared. MobileNetV2 (Sandler et al., 2018) is the primary model and the deployment candidate, chosen for its small size and its use of depthwise separable convolutions, which reduce the number of operations required per image. ResNet50 (He et al., 2016) is the comparison model and serves as an accuracy reference: it is a substantially larger and more conventional residual network, and the question the study asks is what is given up by preferring the smaller one.'));
...
c.push(P('The collection was designed as a paired comparison rather than as a sample. Each subject was photographed twice, once against a plain background and once against a patterned one, on the same handset and under the same lighting, so that the background is what differs within a pair. Across subjects the handset does vary, and five different phones were used, which widens the range of cameras the set represents at the cost of confounding camera with subject. The paired comparison is unaffected by that, because it is made within a subject. The reason for that design is statistical: on a set of this size, comparing two independent groups of photographs would be uninformative, whereas photographing the same feet under both conditions allows the background to be tested with a paired test on the same number of images. The design was prompted by a single observation made while the prototype was being tested, in which the same healthy feet were classified differently against two different backgrounds.'));
